...
覆盖园区确认父子约束、植株编号归属、物候阶段顺序、必需阶段、完成后不可编辑、
同年比较、对象修订冲突、幂等不产生额外事实、关系检查失败回滚与落盘失败不替换
内存状态；`test_http_business.py` 使用临时数据目录启动真实 HTTP 服务，验证
错误码与跨“重启”持久化结果。

前端业务回归测试（Vitest + Vue Test Utils，挂载真实组件与完整 App，经真实
HTTP 客户端访问按服务端契约工作的内存假后端）：

```bash
npm run test:frontend
```

覆盖日期精度、阶段进度、比较可用条件、冲突反馈，以及三类对抗场景：同一操作
并发提交、第一次写入成功但后续关系检查失败、页面最新版本与接口重新读取不
一致。测试策略、覆盖清单与已知实现矛盾见 `docs/testing.md` 与
`docs/test-findings.md`。

一次执行构建、编译、后端测试、前端测试和三条浏览器工作流：
